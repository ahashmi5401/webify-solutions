...
export default function NewCareerPage() {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (!res.ok) throw new Error("Failed to create career listing");
      const career = await res.json();
      router.push(`/admin/careers/${career.id}`);
    } catch (error) {
      console.error("Failed to create career listing:", error);
    } finally {
      setSaving(false);
    }
  };
    ...
    <div className="space-y-6">
      <div className="flex items-center justify-between">
        <div className="flex items-center space-x-4">
          ...
          <div><h1 className="text-2xl font-bold text-foreground">New Career Listing</h1><p className="text-muted-foreground mt-1">Create a new job opening</p></div>
        </div>
        <Button onClick={handleSubmit} disabled={saving}><Save className="h-4 w-4 mr-2" />{saving ? "Saving..." : "Save Listing"}</Button>
      </div>
